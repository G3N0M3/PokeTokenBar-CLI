...
def get_settings_items(app):
    ...
    items.append((
        1,
        [
            f"  [1] Sprite Resolution:         {BOLD}{current_size} columns{RESET}",
    ...
    items.append((
        2,
        [
            f"  [2] Pokédex Page Size:         {BOLD}{pokedex_size} items{RESET}",
    ...
    items.append((
        3,
        [
            f"  [3] Roster Page Size:          {BOLD}{roster_size} items{RESET}",
    ...
    items.append((
        4,
        [
            f"  [4] Expeditions Page Size:     {BOLD}{expedition_size} items{RESET}",
    ...
    items.append((
        5,
        [
            f"  [5] Bag Page Size:             {BOLD}{bag_size} items{RESET}",
    ...
    items.append((
        6,
        [
            f"  [6] Mega Evo Page Size:        {BOLD}{mega_size} items{RESET}",
    ...
    items.append((
        7,
        [
            f"  [7] Reset Game Progress:       {BOLD}{RED}[DANGER]{RESET}",
    ...
    items.append((
        8,
        [
            f"  [8] Team Rocket Process:       {status_tag}",
    ...
    items.append((
        9,
        [
            f"  [9] Monthly Billing Cycle Day: {BOLD}Day {b_day} of each month{RESET}",
    ...
    if base_tokens > 0:
        total_status = f"{BOLD}{GREEN}Active baseline ({format_tokens(base_tokens)}){RESET}"
    else:
        total_status = f"{BOLD}{YELLOW}Unset (showing lifetime total){RESET}"
    items.append((
        10,
        [
            f"  [10] Total Tokens Status:      {total_status}",
            f"      ➔ Type '{BOLD}tokens init{RESET}' to reset displayed Total Tokens to 0",
            f"      ➔ Type '{BOLD}tokens clear{RESET}' to restore lifetime total display"
        ]
    ))
    ...
    items.append((
        11,
        [
            f"  [11] Term Deposits Page Size:  {BOLD}{cd_size} items{RESET}",
    ...
    items.append((
        12,
        [
            f"  [12] Settings Tab Page Size:   {BOLD}{settings_size} items{RESET}",
